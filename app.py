...
def load_css():
    st.markdown("""
    <style>
        /* Background gradient with better readability */
        .stApp {
            background-image: linear-gradient(to bottom, #1a2a3a, #2c3e50);
            color: #f5f5f5;
        }
...
def generate_recap(story_state):
    ...
    Here are the choices that were made:
    {', '.join(choices_made)}
    """
...
def show_setup():
    ...
    if "selected_genre" in st.session_state:
        ...
        with col1:
            character_name = st.text_input("Name your protagonist:", 
                                          placeholder="Enter a name...",
                                          help="Leave blank for a nameless protagonist")
        
        with col2:
            character_trait = st.selectbox("Character's defining trait:", 
...
def show_ending():
    ...
    if "ending_text" not in st.session_state:
        with st.spinner("Crafting your story's conclusion..."):
            ...
            st.session_state.story_state["path_taken"].append({"type": "ending", "text": ending})
...
# Sidebar content
def show_sidebar():
    st.sidebar.image("https://i.imgur.com/8k0ZDkP.png", use_column_width=True)
